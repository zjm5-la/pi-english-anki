...
- **学习端自动备齐今日新卡**：IELTS App 以 `IELTS_ANKI_CLIENT=desktop` 启动的 RPC 会话，会在打开时后台备齐当天剩余新卡；已有 5 张库存也会继续补足。每天剩余目标扣除今天已展示的计划/定制新卡，现有合格库存和定制队列计入准备进度，Skip replacement 不占每日计划；串行分批生成，每批最多 10 个词/词组加 1 个填空，自动模式保留当天填空目标。`dailyNewLimit=0` 时学习不限量，但自动备课仍以每天最多 22 张为有限目标，并扣除今日已展示数。普通 RPC 仍保留库存不超过 1 张时补至最多 5 张的缓冲策略，CLI 不启用此后台逻辑。生成只入库，不切换当前卡、不提前扣学习额度、不修改已有学习记录，到期复习照常优先。关闭自动检查、当天额度用完或系统因复习负担暂停新增时停止；失败保留已完成批次，30 秒后继续补缺口。每批保存前重新核对剩余目标，重复打开和并发学习不额外堆卡。`stats.auto_refill_status` 发布 `strategy`、状态、`inventory`、`queued`、`target`、原因与重试时间，并每 15 秒刷新存活时间。
...
| `/anki:teach <话题>` | 立即生成最多 5 张词汇卡并在审查后保存，不强制搭配语法填空；当前有卡或今日额度已满也可制卡，保留当前答题位置；已有生成时自动排队 |
...
应用可发送 `/anki:teach --request-id <UUID> <话题>`，通过只读查询 `stats` 中的 `teach_request:<UUID>` 跟踪 `queued / generating / checking / revising / saving / succeeded / failed`。回执包含 `requestId`、`phase`、`itemsAdded`、`topic`、`updatedAt`，成功附 `itemIds`，失败附安全的 `errorCode`。成功回执与新卡在同一事务提交，只有实际入库才算成功；最近 100 条终态回执保留用于重复请求去重，待处理回执不会被清理。手动队列不依赖自动检查开关或今日额度，新增卡在首次展示时才占用学习额度。
